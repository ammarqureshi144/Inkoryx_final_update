...
function embedIkx1(pixels: Uint8ClampedArray, json: string): boolean {
  ...
  // Count usable pixel channels (RGB, no alpha)
  let usable = 0;
  for (let i = 0; i < pixels.length; i++) {
    if (i % 4 !== 3) usable++;
  }
  if (bitsNeeded > usable) return false;
...
type EmbedState =
  | { status: "idle" }
  | { status: "working" }
  | { status: "done"; sealedUrl: string; fileName: string; pubKey: string; payload: object }
...
export function SealEmbed() {
  ...
  const handleFile = useCallback((f: File) => {
    if (!f.type.startsWith("image/")) {
      setState({ status: "error", message: "Please upload a PNG image." });
      return;
    }
    setFile(f);
    ...
    img.onload = () => {
      setPreview(url);
      setDims(`${img.width} × ${img.height} px`);
    };
    img.src = url;
  ...
  const embedAndSeal = async () => {
    ...
    try {
      // 1. Draw image onto canvas (strips EXIF automatically)
      ...
      // 2. SHA-256 hash of raw pixel buffer (post EXIF strip)
      const rawBuffer = await new Promise<ArrayBuffer>((res) =>
        canvas.toBlob((b) => b!.arrayBuffer().then(res), "image/png")
      );
      const hash = await sha256Hex(rawBuffer);

      // 3. Generate RSA keypair + sign hash
      const keypair = await generateKeyPair();
      const signature = await signHash(keypair.privateKey, hash);
      const pubKeyPem = await exportPublicKey(keypair.publicKey);

      // 4. Build JSON payload (matches VerifyPayload type in verify page)
      const payload = {
        client: client.trim(),
      ...
      // 5. Embed into pixels using IKX1 format
      const ok = embedIkx1(imageData.data, json);
      if (!ok) {
        setState({ status: "error", message: "Image is too small to hold the payload. Use a larger image." });
        return;
      }
      ctx.putImageData(imageData, 0, 0);

      // 6. Export sealed PNG
      const sealedBlob = await new Promise<Blob>((res) =>
        canvas.toBlob((b) => res(b!), "image/png")
      );
      const sealedUrl = URL.createObjectURL(sealedBlob);
      const baseName = file.name.replace(/\.[^.]+$/, "");

      setState({ status: "done", sealedUrl, fileName: baseName + "_sealed.png", pubKey: pubKeyPem, payload });
    } catch (err: any) {
      setState({ status: "error", message: err?.message ?? "Unknown error" });
    }
  };

  const downloadPubKey = (pubKey: string) => {
    const blob = new Blob([pubKey], { type: "text/plain" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "inkoryx_public_key.pem";
    a.click();
  };
    ...
    <div className="min-h-screen p-8 max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm">
          Embed a cryptographic ownership seal into the image before delivering to the client.
        </p>
      </div>
      ...
      <div
        ...
        className={`cursor-pointer rounded-2xl border-2 border-dashed transition-all duration-300 p-8 text-center glass mb-6 ${
          dragOver ? "border-neon shadow-[0_0_40px_rgba(56,189,248,0.3)]" : "border-neon/30 hover:border-neon/60"
        }`}
      >
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input
            className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground focus:outline-none focus:border-neon/60"
      ...
      <button
        disabled={!file || !client.trim() || !assetId.trim() || state.status === "working"}
        onClick={embedAndSeal}
      ...
      >
        {state.status === "working" ? (
          <><Loader2 className="w-4 h-4 animate-spin" /> Embedding seal…</>
        ) : (
          <><ShieldCheck className="w-4 h-4" /> Embed Seal & Download</>
        )}
      </button>

      {/* Error */}
      {state.status === "error" && (
        <p className="mt-4 text-sm text-red-400 text-center">{state.message}</p>
      )}
      ...
      {state.status === "done" && (
        <div className="mt-6 rounded-2xl border border-green-400/40 bg-green-400/5 p-6">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Send the sealed PNG to your client. Do NOT re-export, compress, or screenshot it — this destroys the seal.
            Save the public key — you need it if you ever want to verify signatures server-side.
          </p>
          <div className="flex gap-3">
            <a
              href={state.sealedUrl}
              download={state.fileName}
              className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg bg-green-400/20 border border-green-400/40 text-green-400 text-sm font-medium hover:bg-green-400/30 transition"
            >
              <Download className="w-4 h-4" /> Download sealed PNG
            </a>
            <button
              onClick={() => downloadPubKey((state as any).pubKey)}
              className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg border border-border text-muted-foreground text-sm hover:border-neon/40 transition"
            >
              <KeyRound className="w-4 h-4" /> Download public key
            </button>
          </div>
        </div>
      )}
